...
from baseballTop import BaseballTop
import time
...
class App(Frame):
    def __init__(self, winname='Baseball Catcher'):  # GUI Design
        ...
        ## My variables
        self.baseball_top = BaseballTop()  # create an instance of the baseballTop class
        self.offset_x = 0.0
        self.offset_y = 0.0
        self.pause = False
        self.pause_time = 0.0
            ...
            if self.processFlag:
                ...
                if self.catchBallFlag:
                    if CATCHER:
                        ...
                        lframe = cv.cvtColor(lframe, cv.COLOR_BGR2GRAY)
                        rframe = cv.cvtColor(rframe, cv.COLOR_BGR2GRAY)
                        # add your code here to detect the ball
                        if not self.pause:
                            self.baseball_top.set_offset((5.6, 7.5))

                            ret, x0, y0 = self.baseball_top.main(lframe, rframe)
                            if ret == -1:
                            ...
                            if time.time() - self.pause_time > 2.0:
                                self.pause = False
                                self.baseball_top = BaseballTop(lframe, rframe)  # reset the baseball_top instance
